...
import music from "../../images/musicgame.jpg";
...
const Image = styled.div(props => [
  ...
  tw`rounded bg-cover bg-center h-64 sm:h-64 lg:h-72`
]);

const ControlContainer = tw.div`absolute bottom-0 right-0 bg-gray-100 px-4 py-2 rounded-tl-3xl border`;
...
const SubDescription = tw.p`max-w-md text-center mx-auto lg:mx-0 lg:text-left lg:max-w-none leading-relaxed text-sm sm:text-base lg:text-lg font-medium mt-2 text-secondary-100`;
const QuoteContainer = tw.div`relative mt-5 lg:mt-10`;
const Quote = tw.blockquote`text-center lg:text-left text-sm sm:text-lg lg:text-xl xl:text-2xl`;
const CustomerInfo = tw.div`mt-2 flex flex-col sm:flex-row items-center justify-center lg:justify-start`;
...
const QuotesLeft = tw(QuotesLeftIcon)`w-4 h-4 opacity-75 text-primary-500 inline-block mr-1 -mt-3`;
const QuotesRight = tw(QuotesRightIcon)`w-4 h-4 opacity-75 text-primary-500 inline-block ml-1 -mt-3`;
...
export default ({
  subheading = "",
  heading = "遊戲簡介",
  description = "多樣化的遊戲內容，讓遊戲不再單調無趣而充滿了重複可玩性！",
  testimonials = null,
  textOnLeft = false
}) => {
  /*
  ...
  const defaultTestimonials = [
    {
      imageSrc: test,
      quote:"心理測驗",
      introduction:"透過心理測驗，搭配出僅屬於你的顏色。"
    },
    {
      imageSrc: closet,
      quote:"換裝系統",
      introduction:"五種髮型、三種眼型以及四套服裝，讓你打造最符合自己印象的角色。"
    },
    {
      imageSrc: adventure,
      quote:"卷軸冒險",
      introduction:"在2D的世界跑跳，體驗不同的冒險！。"
    },
    {
      imageSrc:music,
      quote:"音樂遊戲",
      introduction:"音樂遊戲對戰系統，讓冒險高潮迭起。"
    }
  ];
                ...
                <TestimonialTextSlider arrows={false} ref={setTextSliderRef} asNavFor={imageSliderRef} fade={true}>
                  {testimonials.map((testimonial, index) => (
                    <TestimonialText key={index}>
                      ...
                      <CustomerInfo>
                        <SubDescription>{testimonial.introduction}</SubDescription>
                      </CustomerInfo>
                    </TestimonialText>
                  ))}
